...
public class GestEquipamentoFacade implements IGestEquipamento {
	...
	public Map<String,Equipamento> getEquipamentos(){
		return equipamentos.entrySet().stream().collect(Collectors.toMap(k -> k.getKey(), v -> v.getValue()));
	}
	...
	public Map<String,FichaEquipamento> getFichaEquipamentos(){
		return this.fichaEquipamentos.entrySet().stream().collect(Collectors.toMap(k -> k.getKey(), v -> v.getValue()));
	}
	...
	public Map<String,Orcamento> getAllOrcamentos() {
		Map<String, Orcamento> res = new HashMap<>();
        for(Orcamento or : this.orcamentos.values()){
                res.put(or.getIdOrcamento(),or);
        }   
        return res;
	}
